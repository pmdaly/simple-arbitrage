...
describe("UniswappyV2EthPair", function () {
  ...
  it("get token output from input", function () {
    expect(wethPair.getTokensOut(TOKEN_ADDRESS, WETH_ADDRESS, BigNumber.from("52789948793749671"))).to.equal(
      ETHER.div(10).add(1),
    );
    expect(wethPair.getTokensOut(WETH_ADDRESS, TOKEN_ADDRESS, BigNumber.from("222890894906943052"))).to.equal(
      ETHER.div(10),
    );
  });
});
