...
describe("UniswappyV2EthPair", function () {
  ...
  it("get token input required for output", function () {
    expect(wethPair.getTokensIn(TOKEN_ADDRESS, WETH_ADDRESS, ETHER.div(10))).to.equal(
      ethers.BigNumber.from("52789948793749671"),
    );
    expect(wethPair.getTokensIn(WETH_ADDRESS, TOKEN_ADDRESS, ETHER.div(10))).to.equal(
      ethers.BigNumber.from("222890894906943052"),
    );
  });
  it("get token output from input", function () {
    expect(wethPair.getTokensOut(TOKEN_ADDRESS, WETH_ADDRESS, ethers.BigNumber.from("52789948793749671"))).to.equal(
      ETHER.div(10).add(1),
    );
    expect(wethPair.getTokensOut(WETH_ADDRESS, TOKEN_ADDRESS, ethers.BigNumber.from("222890894906943052"))).to.equal(
      ETHER.div(10),
    );
  });
});
